...
class DualResTensor(object):   
    ...
    def _to_blocks_with_metadata(cls, x: torch.Tensor, metadata: DualResMetaData) -> DualResTensor:
        with timings.env('dualrestensor/to_blocks'):
            ...
            size_lr = (max(metadata.nlowres, 1), C, block_size//cls.LOWRES_FACTOR, block_size//cls.LOWRES_FACTOR)
            ...
            with timings.env('dualrestensor/split_func'):
                data_hr, data_lr = blocksplit.SplitFunction.apply(x, data_hr, data_lr, metadata.map_hr, metadata.map_lr, metadata.block_idx, block_size, cls.LOWRES_FACTOR)    
...
def padded_functions(func, *args, **kwargs):
    ...
    if padding is None:
        padding = args[4] if len(args) > 4 else 0 # for some functions, padding is stored in args[4], but this is not a safe assumption
...
def batch_norm_functions(func, *args, **kwargs):
    ...
    if not kwargs['training']:
        ...
        with timings.env('dualrestensor/custom_batch_norm'):
            ...
            out_hr, out_lr = batchnorm.block_batch_norm(dualres, running_mean, running_var, weight, bias, training, momentum, eps)
            out = DualResTensor(highres=out_hr, lowres=out_lr, metadata=dualres.metadata)
    return out
